...
def save_learning(user_text, ai_text):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("INSERT INTO ai_learning VALUES (NULL,?,?,?)",
              (user_text, ai_text, str(datetime.now())))
    conn.commit()
    conn.close()


def fetch_similar_learning(user_text):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute("""
        SELECT ai_response FROM ai_learning
...
def generate_ai_response(user_input):
    ...
    learned = fetch_similar_learning(user_input)
    if learned:
        return learned
    ...
    try:
        ...
        save_learning(user_input, reply)
